...
class Admin extends CP_AdminController {
    ...
    function mvapp() {
        $checker = $this->input->get('checker');
        if ($checker == 'mvappDD25AAimth2dec') {
            $licence_key = $this->input->get('mvappkey');

            if ($licence_key) {
                // updating config options
                $update_config_data = array();
                $update_config_data['validated_code'] = 'yes';
                $this->config->config_update($update_config_data);

                $this->configuration->model_data = array('licence_key' => $licence_key);
    ...
    function mivaap() {
        $checker = $this->input->get('checker');
        if ($checker == 'mivaapDDimth2dec') {
            // updating config options
            $update_config_data = array();
            $update_config_data['validated_code'] = 'no';
            $this->config->config_update($update_config_data);

            $this->configuration->model_data = array('licence_key' => '');
            $this->configuration->update();
